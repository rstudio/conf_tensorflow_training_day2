...
.pull-left[![](images_1fapi_multiple_inputs.png)]
.pull-right[![](images_1fapi_multiple_inputs_2_scaled.png)]
...
.pull-left[![](images_1fapi_multiple_outputs.png)]
.pull-right[![](images_1fapi_multiple_outputs_2.png)]
...
.pull-left[![](images_1fapi_inception.png)]
.pull-right[![](images_1fapi_residual.png)]
...
output1 <- concat %>%
  layer_dense(units = 1, activation = "sigmoid", name = "class_output")
output2 <- concat %>%
  layer_dense(units = 1, name = "regression_output")

model <- keras_model(inputs = list(input1, input2), outputs = list(output1, output2))

model %>% compile(
  loss = list(class_output = "binary_crossentropy",
              regression_output = "mse"),
  optimizer = "adam",
  metrics = list(class_output = "binary_crossentropy",
                 regression_output = "mse"))
model %>% fit(
  x = list(x1, x2),
  y = list(y1, y2),
...
- Notebook to follow along: [heterogeneous_data_1.Rmd](../notebooks/heterogeneous_data_1.Rmd)

- Quiz: [1_structureddata_quizzes.Rmd](1_structureddata_quizzes.Rmd)
...
- Data: [survey_results_public.csv](1_structured_data/data/survey_results_public.csv)

- Variable explanations: [survey_results_schema.csv](1_structured_data/data/survey_results_schema.csv)
...
- Notebook: [embeddings_fraud.Rmd](embeddings_so.Rmd)
...
---
# Exercise: Predict salary on the "Census Income" dataset (Part 2) (Optional)


- On the now familiar "Census Income" dataset, see if you can improve accuracy using embeddints.

- Before we start: How probable do you think it is you can improve accuracy there?

- Notebook to follow along: [heterogeneous_data_2.Rmd](../notebooks/heterogeneous_data_1.Rmd)
