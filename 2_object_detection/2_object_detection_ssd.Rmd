...
```{r}
model %>% load_model_weights_hdf5("weights_focal.02-4.29.hdf5")
```
...
```{r}
y_pred <- k_constant(c(-10,0,10))
#y_true <- k_constant(c(0,0,0))
y_true <- k_constant(c(1,1,1))

class_loss_focal(y_true, y_pred) %>% k_eval()

tf$nn$sigmoid_cross_entropy_with_logits(labels = y_true, logits = y_pred) %>% k_eval()
```
