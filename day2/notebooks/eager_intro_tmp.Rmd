...
library(ggplot2)
train_indices <- sample(1:nrow(mpg), 0.8 * nrow(mpg))
x_train <-
  mpg[train_indices, c("displ")] %>% as.matrix()
x_train <- k_constant(x_train)
y_train <-
  mpg[train_indices, c("cty")] %>% as.matrix()
y_train <- k_constant(y_train)
...
mpg_regression_model <- function(name = NULL) {
  keras_model_custom(name = name, function(self) {
...
optimizer <- tf$train$RMSPropOptimizer(learning_rate = 0.3)
...
if (!restore) {
  for (i in seq_len(n_epochs)) {
    ...
    cat(k_shape(gradients[[1]]) %>% as.numeric(),
        ",",
        k_shape(gradients[[2]]) %>% as.numeric(),
        ",",
        k_shape(gradients[[3]]) %>% as.numeric(),
        ",",
        k_shape(gradients[[4]]) %>% as.numeric(),
        "\n")
